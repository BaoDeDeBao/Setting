    ...
    override fun onBindViewHolder(holder: FunctionViewHolder, position: Int) {
        ...
        holder.setOnItemClickListener(object: ItemClickListener {
            override fun onItemClickListener(v: View, position: Int) {
                ...
                if(arrayFunction[position].name.equals("Email")) {
                    var intentEmail = Intent(context, EmailActivity::class.java)
                    context.startActivity(intentEmail)
                }
                else if (arrayFunction[position].name.equals("Change Password")) {
